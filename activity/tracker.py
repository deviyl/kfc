...
class TornActivityTracker:
    ...
    def log_activity_snapshot(self):
        ...
        # Remove members who are no longer in the faction
        current_member_ids = {str(int(uid)) for uid in data["members"].keys()}
        members_to_remove = [uid for uid in members_data.keys() if uid not in current_member_ids]
        for uid in members_to_remove:
            del members_data[uid]
            print(f"Removed departed member: {members_data.get(uid, {}).get('name', 'Unknown')} ({uid})")
